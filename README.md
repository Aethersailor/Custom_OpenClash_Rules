...
> [!IMPORTANT]
> **关于 DNS 泄漏防护：使用本项目的 OpenClash 设置方案，并选择三种订阅路径中的任意一种（订阅转换、远程 YAML 覆写或手工导入 YAML），理论上可以实现 100% 的 DNS 泄漏防护。**
>
> 这里的「理论上 100%」以 Fake-IP、流量接管和 DNS 设置全部按 Wiki 生效，且终端 DNS 请求和相关流量均经过 OpenClash 为前提。客户端启用私有 DNS 或 DoH、设备或部分流量未被接管、插件或固件行为与文档不一致，或自行覆写 DNS 配置时，不属于上述前提，需要单独处理并实际验证。
