...
> [!IMPORTANT]
> **DNS 泄漏防护必须同时使用本项目的 OpenClash 设置方案和一种完整配置路径。** 严格按照 Wiki 完成 Fake-IP、流量接管和 DNS 设置，并使用本项目的订阅转换模板、远程 YAML 覆写模块或 YAML 配置后，非直连域名会交由代理出站侧解析，可阻断本方案覆盖范围内的常见本地 DNS 泄漏路径。仅导入模板或 YAML 不能替代 OpenClash LuCI 设置。
>
> 本项目无法对所有设备和网络环境作出「100% 杜绝 DNS 泄漏」的无条件保证。客户端私有 DNS 或 DoH、未被 OpenClash 接管的流量、插件或固件差异，以及用户额外覆写都可能绕过本方案。部署后应实际检查 DNS 请求路径和 DNS 泄漏测试结果。
