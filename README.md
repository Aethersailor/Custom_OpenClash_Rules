...
> 7. 本项目以 CC-BY-SA-4.0 许可协议发布（见下方许可章节）。在遵守该许可协议及适用法律法规的前提下，使用者依法享有复制、分享和改编权利。出于内容时效性、上下文完整性与合规风险考虑，维护者倡议优先分享本仓库原始链接，不建议脱离原始语境的片段化搬运；该倡议不构成对 CC-BY-SA-4.0 所授予权利的附加限制。
