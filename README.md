...
<p align="center">
	<img src="https://img.shields.io/github/stars/Aethersailor/Custom_OpenClash_Rules?style=for-the-badge&logo=github" alt="GitHub stars">
   	<a href="https://t.me/custom_openclash_rules">
		<img src="https://img.shields.io/badge/dynamic/json?style=for-the-badge&label=频道&logo=telegram&query=$.data.totalSubs&url=https://api.spencerwoo.com/substats/?source=telegram&queryKey=custom_openclash_rules" alt="Telegram">
	</a>
...
* 支持节点按地区分类测速优选；  
* 默认中国大陆流量直连，国外流量走机场节点；
* 中国大陆境内域名和 IP 绕过 Clash 内核，提升访问速度和下载性能，并采用运营商 DNS 解析取得最佳解析结果；
* 海外域名和 IP 使用远端节点服务器的 DNS 进行解析，取得最佳解析结果；  
* 中国大陆境内域名返回真实 IP，海外域名返回 Fake-IP；
* 媒体服务（Youtube、Netflix、Disney+ 等）走指定区域测速选优或指定节点，特定域名（电报、ChatGPT 等）走指定区域节点测速选优或指定节点；  
...
## 关于 DNS 泄露  
配合上述订阅转换模板和方案正确设置 OpenClash 后，中国大陆境内域名将使用国内 DNS 进行解析，默认为运营商DNS，亦可自行设置其他国内 DNS，且大陆域名绕过 Clash 内核，可以返回真实 IP  
海外域名自动通过节点远端默认 DNS 解析，一般为机场默认的 DNS 或者你在 VPS 中设置的 DNS。  
