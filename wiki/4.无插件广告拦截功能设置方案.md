...
> **本方案已停止维护，本页面仅作存档。**
>
> 本页面所述的广告拦截功能已不再推荐使用。该方案可能存在兼容性问题，并且不再提供后续更新。
>
> **推荐替代方案：**
> 建议安装 `luci-app-adblock-fast`（直接安装 `luci-i18n-adblock-fast-zh-cn` 即可）。
> 该插件同样基于 dnsmasq 实现拦截，但提供更完善的 GUI 管理界面和更稳定的规则更新机制。插件不常驻后台，通常不会产生持续的后台进程开销。
> 本项目维护者也是 `adblock-fast` 和 `luci-app-adblock-fast` 仓库的协作者之一。
> 使用上述插件时如遇问题，可以向本项目维护者反馈。
...
本方案通过 OpenClash 的「开发者选项」，在插件启动时运行自定义 Shell 脚本。脚本会自动拉取远程广告过滤规则文件（dnsmasq 格式），并将其加载到系统的 dnsmasq 服务中，从而在不安装额外插件的情况下实现广告拦截与隐私防护。

### ⚠️ 风险提示

1. **稳定性风险：** 脚本依赖 `curl` 拉取远程文件，执行结果容易受到网络环境影响，可能导致拉取失败或 dnsmasq 启动异常。
2. **维护责任：** 如果仍要使用本方案，应具备一定的故障排查能力，例如查看日志和分析脚本错误。项目维护者不对使用本方案导致的网络故障负责。
3. **效果预期：** 本方案仅实现 DNS 层面的域名屏蔽。对于视频贴片广告和网页内嵌脚本广告，建议配合浏览器扩展（如 uBlock Origin），以提高拦截效果。

---

## 📜 2. 规则源说明

> 以下规则源仅根据维护者的个人测试和使用偏好列出。

### 2.1 广告过滤规则库

| 规则名称 | 简介 | 特点 |
| :--- | :--- | :--- |
| **[anti-AD](https://github.com/privacy-protection-tools/anti-AD)** | 中文广告过滤列表。 | 🎯 **精准：** 致力于高命中率与低误杀率，兼顾隐私保护。 |
| **[adblockfilters](https://github.com/217heidai/adblockfilters)** | 规则合集；历史上每 8 小时更新一次。 | 📚 **全面：** 上游包含大量常见规则，自带去重机制。 |
| **[adblockfilters-modified](https://github.com/Aethersailor/adblockfilters-modified)** | 本项目维护者定制的版本。 | 🐋 **巨量：** 集成 anti-AD、OISD、1Hosts 等，历史条目数曾超过 65 万。**误杀率较高，慎用。** |
| **[秋风广告规则](https://github.com/TG-Twilight/AWAvenue-Ads-Rule)** | 轻量化广告规则列表。 | 🍃 **轻量：** 专注于移动端与家用环境，流量节省效果显著。 |
...
> anti-AD ⊇ 秋风广告规则
>
> 请根据设备性能和拦截需求，从上述广告过滤规则中选择一项，无需重复添加。

### 2.2 加速规则

- **[GitHub520](https://github.com/521xueweihan/GitHub520)：** 通过 `hosts` 改善 GitHub 访问，缓解图片加载失败等问题。
...
部署前，进入「网络」→「DHCP/DNS」页面，**取消勾选**「DNS 重定向」。

- **原因：** 如果启用重定向，DNS 请求可能被重定向并绕过 dnsmasq，导致广告过滤失效。
- **注意：** LEDE 源码固件可能不提供此选项，此时可以忽略该步骤。

<img width="718" alt="Redirect" src="https://github.com/user-attachments/assets/2af516d8-271d-411b-9039-3cf2210b5a5e" />

---

## 🛠️ 4. 部署方法

请从 **一键脚本** 和 **手动配置** 中选择一种部署方式。

### 4.1 方式一：一键脚本（SSH）

> [!WARNING]
> **该一键脚本入口已经下线，不再提供可执行安装命令。** 历史脚本仅保存在主仓库的 `shell/archived/` 目录中，用于历史审阅，不应再用于新部署。

<img width="467" alt="one-key" src="https://github.com/user-attachments/assets/0f470b60-3d3d-4d3f-936d-ca6bd6bc4987" />
...
3. 将下方任一脚本**完整粘贴**到 `exit 0` **之前**。

<img width="757" alt="dev" src="https://github.com/user-attachments/assets/5561f6ba-f4d8-4ecb-aaef-b56043bd2c23" />

---

#### 📋 脚本模板选择

> 点击下方标题可展开查看历史脚本代码。以下选项均已停止维护，不再推荐部署。

<details>
<summary><strong>历史选项 1：anti-AD 与 GitHub520</strong></summary>
...
<details>
<summary><strong>历史选项 2：原版 adblockfilters 与 GitHub520</strong></summary>
...
<details>
<summary><strong>历史选项 3：adblockfilters-modified 与 GitHub520</strong></summary>
...
<details>
<summary><strong>历史选项 4：秋风广告规则与 GitHub520</strong></summary>
...
<details>
<summary><strong>历史选项 5：仅添加 GitHub520 加速规则</strong></summary>
...
如果功能异常或不再需要，请进入 OpenClash 的「开发者选项」，**删除前述添加的代码**，并保留默认内容。
