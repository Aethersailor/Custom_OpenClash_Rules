...
> [!WARNING]
> 不要把 `--force` 作为常规升级参数。它用于强制处理软件包降级或“没有检测到变更”等特殊情况；只有在理解检查结果并确认确有必要时才应使用。
...
- 各自子网公网出口独立
- 不合并 LAN 广播域，不会自动转发二层广播；是否允许主动访问或扫描对端地址，由 Tailscale 访问控制策略决定
- 仅允许通过明确 IP 访问对端
- 子网设备增删无需调整防火墙规则
- 不需要在公网开放任何端口
...
1. 在 Machines 页面分别批准双方通告的 subnet routes。
2. 在 Access controls 页面确认 ACL/grants 允许两个子网互访。

> [!IMPORTANT]
> **路由批准与访问控制是两个独立条件。** 批准 route 只决定路由是否下发，ACL/grants 决定对应流量是否允许通过。两者缺一不可。

如果 tailnet 已使用自定义访问控制策略，可按实际子网加入双向 grants，例如：
...
若已有其他 grants，请将上述规则合并进现有 `grants` 数组，不要直接覆盖整个策略文件。需要限制端口时，可将 `*:*` 改为明确的协议和端口范围。
