...
> [!TIP]
> 固件大小上限受所用 ASU 服务器限制。OpenWrt 官方服务器当前通常允许的最大值为 1024 MB；执行前请在服务器状态信息中确认 `Maximum requested root filesystem size`。
...
批准子网路由后，确认现有 tailnet 访问控制策略允许两个子网双向通信。如果现有策略已经允许，例如仍使用默认全放行策略，则无需修改；否则必须在 Access controls 中添加双向 grants。
...
再从 B 子网设备反向测试 A 子网设备，并使用 `traceroute` 或 `tcpdump -ni tailscale0` 确认流量经过 `tailscale0`。如果小包可通，但部分网页或大包连接异常，再检查路径 MTU/MSS；不要先盲目放宽防火墙。
