...
- GitHub 仓库和 Raw 文件可访问，但 jsDelivr 与 Raw 不一致或明确不存在；同时备用校验文件与 Raw 完全一致。

其他情况记为未知。`403`、`429`、请求超时、`5xx`、备用快照落后以及证据不足均不会触发切换。未知状态会清空连续计数。连续 3 次失败后停用跳转规则，连续 3 次成功后重新启用。
