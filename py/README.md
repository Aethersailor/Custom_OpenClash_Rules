...
| [`generate_rules.py`](generate_rules.py) | 从 `rule/` 中的 5 个 `.list` 来源和 `game_rule/*.list` 生成 Domain、IP、Classical 和端口 YAML；传入 Mihomo 可执行文件时同时生成非空 MRS | `rule/*_Domain.*`、`rule/*_IP.*`、`rule/*_Classical*.yaml`、`game_rule/*_Domain.*`、`game_rule/*_IP.*`、`game_rule/*_Classical*.yaml` |
