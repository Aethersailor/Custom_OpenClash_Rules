...
- **[check_config_consistency.py](check_config_consistency.py)**: 校验活动 INI/YAML 的规则周期、正常/回退规则顺序、策略组引用和 MRS 格式，并确保 `Custom_Clash_Mainland.ini` 与唯一维护源 `Custom_Clash.ini` 一致。修改主配置后使用 `--sync` 同步兼容文件，CI 使用 `--check` 防止漂移。
...
- **`archived/`**: 存放已废弃或不再使用的历史脚本。[查看详情](archived/)
