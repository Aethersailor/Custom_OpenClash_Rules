...
注意：2025年7月1日更新，有些固件无法正常的运行超长的 curl 命令，导致拉取广告过滤规则失败并导致 dnsmasq 工作不正常，由于我本人使用的固件没这个问题，所以无法测试和修改。如果遇到拉取报错，请删除去广告代码。  
...
规则数量：65万+
本项目维护者在原版 adblockfilters 的分支上维护的增强版，在原版规则的基础上，增加了 anti-ad、OISD Big、PeterLowe 等大量流行的规则作为上游，保留了原项目的去重和去无效域名机制，巨量体积，数十万条规则有效治疗规则焦虑症。  
