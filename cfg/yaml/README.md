...
| **② 远程 YAML 覆写模块** | 无需订阅转换；填写模块变量即可下载对应 YAML 并写入订阅；远程文件可随仓库维护更新 | 需要了解 OpenClash 覆写模块的添加、变量填写和排障；远程更新可能改变下一次加载结果 | 希望使用 YAML 和自动更新，但不需要长期手工编辑文件的用户 |
...
手工 YAML 自由度最高，但仓库后续更新不会自动合并到本地副本。升级时应备份旧配置，对比迁移 Provider、策略组和规则变化。
