...
每个游戏必须使用独立文件夹；同一游戏的不同大区分别使用独立 `.list`。修改规则时，只编辑对应的 `.list` 来源文件，不要直接编辑 YAML 或 MRS 派生文件。引用时，根据所选文件配置匹配的 `behavior` 和 `format`，再在 `rules` 中添加目标 `RULE-SET`。具体策略名称和规则顺序应根据现有配置确定。
