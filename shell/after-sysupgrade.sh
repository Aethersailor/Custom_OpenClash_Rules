...
main() {
	...
	TARGET_FILE_1="/etc/openclash/custom/openclash_custom_overwrite.sh"

    # 备份原始文件
    cp "$TARGET_FILE" "${TARGET_FILE}.bak"

	# 修改 覆写设置 > 开发者选项
	# 修改文件内容：找到特定行并替换成新的内容
	sed -i "/^#ruby_edit \"$CONFIG_FILE\" \"\['experimental'\]\" \"{'sniff-tls-sni'=>true}\"/c\ruby_edit \"$CONFIG_FILE\" \"['experimental']\" \"{'sniff-tls-sni'=>false}\"" "$TARGET_FILE_1"
