...
(
    ...
    LOG_OUT "[广告过滤规则拉取脚本] 拉取最新的 adblockfilters + ad-AD 广告过滤规则，规则体积较大，请耐心等候…"
    mkdir -p /tmp/dnsmasq.cfg01411c.d
    curl -sSL --retry 5 --retry-delay 1 "https://github.boki.moe/https://raw.githubusercontent.com/Aethersailor/adblockfilters/refs/heads/main/rules/adblockdnsmasq.txt" -o /tmp/dnsmasq.cfg01411c.d/adblockfilters+anti-ad-for-dnsmasq.conf 2> /tmp/adblockfilters+anti-ad-curl.log

    if [ $? -eq 0 ]; then
        LOG_OUT "[广告过滤规则拉取脚本] adblockfilters + ad-AD 规则拉取成功！"
    else
        LOG_OUT "[广告过滤规则拉取脚本] adblockfilters + ad-AD 规则拉取失败，查看 /tmp/adblockfilters+anti-ad-curl.log 获取详细信息。"
    fi
...
echo "adblockfilters + ad-AD 广告过滤规则和 Github520 Hosts 拉取代码已写入到“开发者选项”文件 $TARGET_FILE 中，并将在 OpenClash 下次启动时生效！"
