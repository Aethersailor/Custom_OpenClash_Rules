...
本目录收录与项目主配置相互独立的辅助脚本。除非 README 明确说明，否则这些内容不会被 OpenClash 配置、规则生成流程或 GitHub Actions 自动加载。
...
- `archived/` 中的内容不保证仍然可用或完整，也不保证适配当前的 OpenWrt、dnsmasq、OpenClash 或上游规则格式，切勿直接用于生产环境。
