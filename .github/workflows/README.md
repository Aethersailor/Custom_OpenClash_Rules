...
| [`auto-generate-rules.yml`](auto-generate-rules.yml) | 从 5 个 `.list` 规则源统一生成 `.yaml` 和 `.mrs` 派生规则，并在精确 SHA 校验成功后调用统一 CDN 发布器 | 对应规则源或生成器变更，或手动触发 |
...
| [`purge-jsdelivr.yml`](purge-jsdelivr.yml) | 按 [CDN 发布契约](../jsdelivr-publish.json) 精确刷新并逐字节验证 `@main` 与 `@refs/heads/main` 缓存键；无公开文件变化时自动跳过刷新 | `main` 分支推送、规则生成且校验成功，或手动修复范围 |
